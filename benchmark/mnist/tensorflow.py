...
# parameter samples
batches_samples = [30, 50, 60, 100]  # the best option is lesser batch size (but time consuming)
rates_samples = [0.07, 0.05, 0.03, 0.01]  # differs for each case
epochs_samples = [15, 20, 25, 30]  # the best option is lesser batch size (but time consuming)


# run one round with specific parameters
def run_round(session, batch_size=100, learning_rate=0.01, num_epochs=20):
    summary_writer = tf.summary.FileWriter(f'board/board_{batch_size}_{learning_rate}_{num_epochs}')
    session.run(init)
    global_step = 0
    for epoch in range(num_epochs):
        ...
        for i in range(total_batch):
            ...
            summary, _, c = sess.run([merged_summary, optimizer, cross_entropy],
        ...
        print("Epoch:", '%04d' % (epoch + 1), ", cost:", "{:.9f}".format(avg_cost), file=sys.stderr)
...
best_solution = {"validation_accuracy": 0}
with tf.Session() as sess:
    for batch, rate, epochs in itertools.product(batches_samples, rates_samples, epochs_samples):
        sol = run_round(sess, batch, rate, epochs)

        if best_solution["validation_accuracy"] < sol["validation_accuracy"]:
            best_solution = sol
...
with tf.Session() as sess:
    sess.run(W.assign(best_solution["W"]))
    sess.run(b.assign(best_solution["b"]))

    test_accuracy = accuracy.eval({x: test_images, y: test_labels})

print(
    f'Best Accuracy on Validation = {best_solution["validation_accuracy"]}: batch_size={best_solution["batch_size"]},'
    f' learn_rate={best_solution["learning_rate"]}, num_epochs={best_solution["num_epochs"]}')
